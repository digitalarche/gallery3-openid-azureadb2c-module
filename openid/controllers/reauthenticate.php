...
class Reauthenticate_Controller extends Controller {
  public function index() {
    ...
    if ($is_ajax) {
      $identity = openid::find_recent_identity( identity::active_user()->id );
      if($identity->loaded()) {
        if( strcmp($identity->provider,"facebook") == 0 ) {
          $valid = openid::process_facebook();
        } elseif( strcmp($identity->provider,"microsoft") == 0 ) {
          $valid = openid::process_microsoft();
        } else {
          $valid = openid::process_openid($identity->provider);
        }
        if($valid) {
          if (!request::is_ajax()) {
  ...
  private static function _show_form($form) {
    ...
    if($identity->loaded()) {
      ...
      if($debug) {
        $fb_provider = strcmp($identity->provider,'facebook') == 0 ? "true" : "false";
        Kohana_Log::add("information", "[OpenID Module] Provider: '{$identity->provider}' (facebook? {$fb_provider}) [reauthenticate::_show_form()]\n");
      }
